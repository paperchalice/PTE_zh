...
给定空间\((\Omega, \mathcal{F}, \mu)\)，若\(N \in \mathcal{F}\)并有\(\mu(N) = 0\)，则称\(N\)为\term{\(\mu\)零测（度）集}，若\(\mu\)在上下文中可以确定，则有时会省略\(\mu\)。本节的主要目的是证明以下内容：
...
\end{theorem}
\begin{proof}
	\lemref{lem:1.1.7}说明了\(\bar{\mathcal{S}}\)中的元素是\(\mathcal{S}\)中元素的有限不交并。
...
\begin{enumerate}
	...
	\end{itemize}
\end{enumerate}
则称\(\alpha\)为\((\Omega, \mathcal{F})\)上的\term{带号测度}（signed measure）
...
\begin{example} \label{ex:a.4.1}
	令\(\mu\)为一测度，若函数\(f\)满足\(\int_\Omega f^-\,\dif \mu < +\infty\)，根据\ref{exe:1.5.4}，\(\alpha(A) = \int_A f\,\dif\mu\)是一个带号测度。% FIXME: 原文也没说清楚是哪个练习。。。
\end{example}
\begin{example} \label{ex:a.4.2}
	令\(\mu_1\)、\(\mu_2\)为测度且有\(\mu_2(\Omega)<+\infty\)，则\(\alpha(A)=\mu_1(A) - \mu_2(A)\)是一带号测度。
\end{example}

若尔当分解定理，即\thmref{thm:a.4.6}，会说明\exref{ex:a.4.2}是带号测度的一种表示形式。
...
\begin{exercise}
	\item 证明在\exref{ex:a.4.1}中，\(A\)是正集合当且仅当\(\mu(A\cap \{x:f(x)< 0\}) = 0\)。
\end{exercise}
...
\end{theorem}
\begin{proof}
	令\(c=\inf\{\alpha(B): B\text{是负集合}\}\leq 0\)。令\((B_i)\)为一负集列且有\(\alpha(B_i) \downarrow c\)。令\(B = \bigcup_i B_i\)。根据\lemref{lem:a.4.3}，\(B\)是负集合，故根据定义\(\alpha(B) \geq c\)。
...
给定两个测度\(\mu_1\)和\(\mu_2\)，如果存在集合\(A\)使得\(\mu_1(A)=\mu_2(A^c)=0\)则称这两个测度\term{互为奇异测度}（mutually singular）。此时我们也称\(\mu_1\)较\(\mu_2\)是\term{奇异}的（singular with respect to (w.r.t.)）并记为\(\mu_1\perp\mu_2\)。
